package com.gamecenter.gamecenter.adapter.NewChatAdapter;
...
import com.gamecenter.gamecenter.util.chatutil.GlideUtils;
...
public class ChatAdapter extends BaseQuickAdapter<Message,BaseViewHolder> {
    ...
    */
    ...
    private void setStatus(BaseViewHolder helper, Message item) {
        ...
        if (msgContent instanceof TextMsgBody || msgContent instanceof AudioMsgBody
                ||msgContent instanceof VideoMsgBody || msgContent instanceof FileMsgBody) {
            //只需要设置自己发送的状态
    ...
    private void setContent(BaseViewHolder helper, Message item) {
        if (item.getMsgType().equals(MsgType.TEXT)){
            ...
            Date dt = new Date(item.getSentTime());
            String sDateTime = sdf.format(dt);  //得到精确到秒的表示：2006-08-31 21:08:00
            helper.setText(R.id.item_tv_time,sDateTime);
            /*
            if(item.getSendername() == null || "".equals(item.getSendername())){
                helper.setText(R.id.tv_sender_name,item.getSendername());
            }else {
                helper.setText(R.id.tv_sender_name,item.getSenderId());
            }
            */
